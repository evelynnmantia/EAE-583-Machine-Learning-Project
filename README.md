...
<p align="center">
  <img src="images/figure_1.jpg" width="850">
</p>
...
With this approach, it is expected that the machine learning model will perform generally well in predicting hail size. Literature assessing hail occurrence and predicting severe and significant hail using machine learning, specifically a random forest, have shown an increase in the skill of hail potential and severity/size (e.g., Burke et al., 2020; Gagne et al., 2017). Since this project is applying a similar method to previous work, it should align with better hail potential and size forecasting. Conversely, due to the limitations of this project in terms of simplicity and scope when compared to the methods of previous literature (e.g., not using a regression model like Gagne et al., 2017), it is possible to have results that do not classify hail size based on environmental variables as well as work that has been done.

## III. Results and Discussion
Our results look at July 31, 2024, at 21Z, which we know to be a date and time in which there were hail threats across Minnesota. The data was filtered to only include MRMS MESH and HRRR data points within this region of the CONUS. The extent of Minnesota is considered to be within 89.49 W to 97.24 W and 43.50 N to 49.38 N. This filtering left the dataset with 44,601 data points to work with when creating its predictions in our random forest regressor model. Out of this, 31,723 samples were used to produce the training dataset. The predefined splits approach was used to separate the training and validation datasets, which were later used to determined the best estimating model based on a combination of 240 candidates. The best parameters were found using a grid search method of predefined parameter grid of varying criteria, and the following classifications, as shown in Figure 2, were found to produce the model: criterion = squared error, maximum depth = 20, minimum leaf samples = 1, mininmum samples split = 2, number of estimators = 100, and random state = 100. 
Using the model defined in Figure 2, the testing dataset was input into the random forest regressor model to receive the final results. Figure 3 shows the success of this model on the testing dataset, which is as follows: R2 score = 0.67, mean squared error = 0.65, and mean absolute error = 0.18. The R2 score determinines how well fit the model is where an R2 of 1 indicates a perfect fit. The mean squared error measures an average of the squared difference between actual and predicted values, while the mean absolute error measures an average absolute difference between actual and predicted values with a value of 0 indicating perfect predictions for both of these. Based on the large spread between the two, we can assume that our dataset may have more outliers to which the mean squared error is much more sensitive to than the mean absolute error, since this value treats all errors linearly in comparison. While the current input data is less than ideal, the functionality of the random forest regressor model is successful and has the potential to be used on a larger date range. 
...
The relationship between the random forest regressor prediction of MESH and the actual MRMS MESH prediction for a given latitude and longitude for the study domain and period shows that our ML model has the tendency to overestimate smaller MESH values while underestimating bigger MESH values (seen in relation to the y=x reference line which would be a perfect fit relationship) (Fig. 4). In order to more fully understand how our model performs, future work, not performed here due to time and scope restrains, may include a comparison between the ML model output MESH and surface hail reports archived by the Storm Prediction Center (SPC). It would be expected that our created random forest regressor MESH output would overestimate the small ground-truth hail sizes while underestimating the larger ground-truth hail sizes (e.g., Vasquez and Lui, 2024).
...
## IV. Summary
This project used a random forest regression model to predict the maximum hail size based on environmental variables from the HRRR. Improving hail forecasting ability by drawing connections between hail sizes and environmental variable values can be used to better prepare industry to anticipate potential damage and loss due to severe hail events, in addition to introducting opportunities for better hail mitigation/preparation. Further, this ML tool can be useful for forecasters to inform and communicate accurate prediction of hail risk/impact on society for a hail event. The random forest regression model used a dataset composed of fourteen HRRR environmental variables aligned with MRMS MESH values at each location to train, validate and test on. We found that our model underperformed with an R2 score of only 0.67 due to the limited time of the project and unexpected data constraints, which only included a single time step for latitude/longitude bounds of Minnesota. An increased date range, as well as an assessment of more regions of the CONUS would likely improve the performance of this model, which could further improve hail size prediction. In the future, the random forest regressor will be run using more data points east of the continental divide from March 1st until May 31st, 2024, which should provide the model with more samples to increase the model performance. In addition to the lengthening of the period and increased regional variability used for input data, more HRRR variables will be included to further increase the ability of the model to predict the maximum hail size.
...
Vasquez, E. R., and C. Liu, 2024: Radar-Derived Convective Features and Atmospheric Environments of Hail-Producing Storms over the Contiguous United States, https://doi.org/10.1175/MWR-D-23-0151.1.
